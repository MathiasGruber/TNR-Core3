...
{function generate_simple_req_display}
    ...
    {foreach $data as $requirement_key => $requirement}
        {if isset($requirement['status']) && substr( $requirement_key, 0, 4 ) === "join"}
            ...
            {if $status === false}
                <font color="red" class="shadow-outline" title="{implode(' and ', $hover)}>&#10007</font> - {implode(' and ', $write_ups)}<br>
            {else}
                <font color="green" class="highlight-outline bold" title="{implode(' and ', $hover)}>&#10003</font> - {implode(' and ', $write_ups)}<br>
            {/if}
...
<div id="quest_widget">
  {if $tracked_quest != '' && $quest_widget == 'yes'}
    ...
    {elseif $quest_widget == 'yes' && count($trackable_quests) > 0}
        <div class="menuContainer">
        <div class="menuBg1 menuBg1-left">
          <div class="menuPerforationVertical menuPerforationVerticalLeft">
            <div class="perforationMenuHorizontal perforationMenuHorizontalTop">
