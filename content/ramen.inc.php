...
class ramen extends eatingLib {
    ...
    private function setRamenMenu(){
        ...
        // Get the village structures which are used to modify ramen prices
        $villageVars = $GLOBALS['database']->fetch_data("
             SELECT `ramen_bonus`,`owned_territories`
             FROM `village_structures`,`villages`
             WHERE
                `village_structures`.`name` = '" . $GLOBALS['userdata'][0]['village'] . "' AND
                `villages`.`name` = `village_structures`.`name`
             LIMIT 1");
        if( $villageVars !== "0 rows" ){
            if( $villageVars[0]['ramen_bonus'] > $GLOBALS['user']->load_time ){
        ...
        // Reduce based on amount of structures
        $percReduction = $villageVars[0]['owned_territories'] * 2.5;
        if( $percReduction > 50 ){
            $percReduction = 50;
        }
        $percReduction = ( 100-$percReduction ) / 100;
        for( $i=0; $i<count($this->menu); $i++ ){
            $this->menu[$i]['cost'] *= $percReduction;
        }
        ...
        // Get clan and reduce cost based on this
        ...
        if( $clanLib->isUserClan($GLOBALS['userdata'][0]['clan']) ){
            ...
            if( $clan[0]['clan_ramen_shop'] > $GLOBALS['user']->load_time ){
                for( $i=0; $i<count($this->menu); $i++ ){
                    $this->menu[$i]['cost'] = $this->menu[$i]['cost'] * 0.9;
                }
            }
        }
        ...
        // Ceil the values
        for( $i=0; $i<count($this->menu); $i++ ){
            $this->menu[$i]['cost'] = ceil( $this->menu[$i]['cost'] );
        }
    }

    // Main screen
    private function main_screen() {
        $this->showRamenMenu( array('message'=>"Buy your ramen here. You currently have ".$GLOBALS['userdata'][0]['money']." ryo on you.",'hidden'=>'yes') );
    }
