...
class page {
    ...
    function load_content() {
        ...
        if (
            ($GLOBALS['userdata'][0]['user_rank'] == "Admin" ||
             $GLOBALS['userdata'][0]['user_rank'] == 'ContentMember' ||
             $GLOBALS['userdata'][0]['user_rank'] == 'ContentAdmin')
        ) {

            if (isset($this->modules[$id][0]) && file_exists(Data::$absSvrPath.$this->modules[$id][0])) {
                //	File found, include it
                require_once(Data::$absSvrPath.$this->modules[$id][0]);
            } else {
    ...
    function load_modules() {
        ...
        while (false !== ($file = readdir($dirHandle))) {
            if ($file != '.htaccess' && $file != '.' && $file != '..') {
                $name = explode('.', $file);
                $this->modules[$i] = array("/panel_content/modules/".$file, $name, $i);
                if ($file == "content.log.class.php") {
                    $this->defaultPage = $i;
                }
                $i++;
            }
        ...
        //$moduleList = array(
        //    array("path"=>"/panel_admin/modules/","file"=>"content.bloodline.class.php"),
        //    array("path"=>"/panel_admin/modules/","file"=>"content.jutsu.class.php"),
        //    array("path"=>"/panel_admin/modules/","file"=>"content.DAM_calculator.class.php")
        //);
        
        // Add to list
        //foreach( $moduleList as $module ){
        //    $name = explode('.', $module['file']);
        //    $path = $module['path'].$module['file'];
        //    $this->modules[$i] = array( $path, $name, $i );
        //    $i++;
        //}
    ...
    function load_menu() {
        $GLOBALS['template']->assign('menuLoad', './panel_content/files/menu.tpl');
    }
