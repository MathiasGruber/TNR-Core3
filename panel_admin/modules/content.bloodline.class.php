...
class bloodline_admin {
    ...
    public function __construct() {
        ...
        // Set permissions
        $this->canEdit = array("Admin", "ContentAdmin");
    ...
    // Show users using this bloodline
    private function showUsers() {
        ...
        if ($data != '0 rows') {
            ...
            $users = $GLOBALS['database']->fetch_data("
                SELECT `bloodlines`.*, `users`.`username`, `users`.`id`
                FROM `bloodlines` 
                LEFT JOIN `users` ON (`users`.`bloodline` LIKE CONCAT(`bloodlines`.`name`,'%'))
                WHERE `bloodlines`.`name` = '" . $_GET['bid'] . "'
                LIMIT " . $min . "," . $number);
            if ($users !== "0 rows" && $users[0]['username'] == null) {
                $users = "0 rows";
            }

            // Parse table
            tableParser::show_list(
                    'item', 'Users who have bloodline: ' . $data[0]['name'], $users, array(
                'id' => 'User ID',
                'username' => "Username"
                    ), array(
    ...
    private function main_screen() {
        ...
        // Create options
        $options = array(array("name" => "Users", "act" => "showUsers", "bid" => "table.name"));
        $topOptions = array();
