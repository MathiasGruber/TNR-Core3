...
class specialsurprise extends blackMarketLib{
    ...
    private function overview() {
        ...
        $surprises = $this->getSpecialSurprises( false, false, true );
        ...
        tableParser::show_list(
            ...
            array(
                ...
                false,
                array(
                    array("name" => "New Surprise", "href" => "?id=" . $_GET["id"] . "&act=new")
                ),
                false, // No sorting on columns
                false, // No pretty options
                false, // No top search field
                'Here you can create special surprises which are sold on the black market.
                 ...
                 separate special surprises may be in the system at once. Grouping is indicated by blue/purple color
                 of the entry names. The chance of each entry in its given group is calculated based on the "frequency" entry on each item'
        );
    }

    // Form for inserting new surprise
    private function new_surprise(){
        tableParser::parse_form('blackmarket_surprises', 'New Surprise', $this->noShowFields() );
    }
    ...
    private function modify_surprise() {
        ...
        if ($data != '0 rows') {
            if ( $data[0]['event_item'] == 'yes' || $GLOBALS['userdata'][0]['user_rank'] == 'Admin' ) {
                tableParser::parse_form('blackmarket_surprises', 'Update Surprise', $this->noShowFields(), $data);
            } else {
